...
export default function NavBar({
  ...
  const changeArrowColors = (buttonSide: number) => {
    if (maxPosition > currentPosition) return NavBarButton_DARK;

    if (buttonSide === 0) return NavBarButton_DARK;
  ...
  return (
    <div className="bg-[#F0F9F1] pt-[20px] w-full justify-around">
      <div className="flex">
        <button
          type="submit"
          className="border-none bg-[#F0F9F1] mr-auto ml-[15px] mt-[15px] rotate-0"
          onClick={() => shiftNavBarSection(-1)}
        >
          <img src={changeArrowColors(0)} alt="leftButton" />
        </button>
        <p className="text-[#9BA3AC] font-Poppins text-[32px] not-italic font-normal leading-[48px] text-center">
          {changeString(currentPosition)}
        </p>
        <button
          type="submit"
          className="border-none bg-[#F0F9F1] ml-auto mr-[15px] mt-[15px]"
          style={rotateArrow()}
        >
          <img src={changeArrowColors(1)} alt="rightButton" />
        </button>
      </div>
      <div className="mt-[20px] bg-[#FFE2CC] w-[7/10] ml-auto mr-auto rounded-[30px]">
        <div
          className="bg-[#FF9C55] rounded-[30px]"
          style={{ width: `${progressBarPercent}%` }}
        >
          &shy;{' '}
          {/* Can't get the div to render without some character here... */}
        </div>
      </div>
    </div>
  );
